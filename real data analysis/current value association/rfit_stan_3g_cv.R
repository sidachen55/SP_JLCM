...
posterior_probs <- final_model$pprob[, c("probYT1", "probYT2", "probYT3")]


init_function <- function() {
  u1 <- runif(1,0.05,0.25)
  u2 <- runif(1,0.05,0.25)
  u3 <- runif(1,0.05,0.25)
  list(
    ...
    alpha2 = runif(1,-5,0), 
    alpha3 = runif(1,-5,0),
    theta1 = (matrix(fit_summary[,"coef"], 3, 4)[1,])* runif(1,0.7,1.4),
    theta2 = (matrix(fit_summary[,"coef"], 3, 4)[2,])* runif(1,0.7,1.4),
    theta3 = (matrix(fit_summary[,"coef"], 3, 4)[3,])* runif(1,0.7,1.4),
    psi1 = rnorm(3,0,1),
    psi2 = rnorm(3,0,1)
  )
}
...
for (chain in 1:6) {
  ...
  HPD_ind <- sort.list(lp_chain,decreasing = TRUE)[1:(0.5*(length(lp_chain)))] 
...
# For beta = 0.8 and beta <=0.5, two different chains are selected; both converge to the same mode, but the chain for beta<=0.5 exhibits better mixing and is used for inference.


posterior <- as.array(fit)
posterior_chain <- posterior[,best_chain, ]  
mcmc_trace(posterior_chain, pars = c("var_e1", "var_e2", "var_e3","alpha1", "alpha2", "alpha3", "beta1[1]","beta1[2]","beta1[3]", "beta2[1]","beta2[2]","beta2[3]", "beta3[1]","beta3[2]","beta3[3]", "lambda1", "lambda2", "lambda3"))
...
save(IC_smy, file=paste0("IC_smy_3g.rdata"))

################################################################################
samples <- extract(fit, pars=c("theta1[1]","theta1[2]", "theta1[3]", "theta1[4]", "theta2[1]","theta2[2]", "theta2[3]", "theta2[4]", "theta3[1]","theta3[2]", "theta3[3]", "theta3[4]",  
                               "beta1[1]", "beta1[2]","beta1[3]", "beta1[4]", "beta2[1]", "beta2[2]", "beta2[3]","beta2[4]", "beta3[1]", "beta3[2]", "beta3[3]", "beta3[4]",  "alpha1", "alpha2",  "alpha3", "lambda1", "lambda2", "lambda3",
                               "var_e1", "var_e2", "var_e3", "var_b1[1]", "var_b1[2]", "var_b1[3]", "var_b2[1]", "var_b2[2]", "var_b2[3]", "var_b3[1]", "var_b3[2]", "var_b3[3]", 
                               "psi1[1]", "psi1[2]", "psi1[3]", "psi2[1]", "psi2[2]", "psi2[3]"), permuted=FALSE)
...
fit_smy <- list( theta1_4 = calc_summary(posterior_samples[, paste0("theta", spm[1], "[4]")]),
                 ...
                 psi20 = calc_summary(posterior_samples[, "psi2[1]"]),
                 psi21 = calc_summary(posterior_samples[, "psi2[2]"]),
                 psi22 = calc_summary(posterior_samples[, "psi2[3]"]),
                 ...
                 D3_11 = calc_summary(posterior_samples[, paste0("var_b", spm[3], "[1]")]),
                 D3_22 = calc_summary(posterior_samples[, paste0("var_b", spm[3], "[2]")]),
                 D3_33 = calc_summary(posterior_samples[, paste0("var_b", spm[3], "[3]")])
)
...
km_plot_clusters <- ggsurvplot(km_fit_cluster,
                               ...
                               conf.int = FALSE,  
                               legend.title = "",
                               legend.labs = c("subgroup 1", "subgroup 2", "subgroup 3"),
