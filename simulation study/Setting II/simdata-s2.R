################################################################################
# Model 2 scenario 2: 2 sub-populations (G=2)
# cov: x3 [independent of age gender]
################################################################################
...
phi_10 <- -0.4  # Intercept for class 1
phi_13 <- 1   # class 1 coefficient for x3
...
x3 <- rnorm(n, 0, 1) # artificial cov for the membership model only 

# generate membership prob matrix
cluster_prob <- matrix(NA, n, 2)
for (i in 1:n) {
  linear_predictor <- phi_10 + x3[i] * phi_13 
