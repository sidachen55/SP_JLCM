################################################################################
# for sim setting 2 (G=2 clusters)
# covariate independent membership submodel (Model 1)
################################################################################
library(rstan)
library(loo)
library(statmod)
...
##########################################
fit <- stan(file = "rstan_2g_mlr0_new.stan", 
...
save(fit_smy, file=paste0("stan_2g_mlr0_new_",task_id,".rdata"))


#############################################
# Create trace plots for specified parameters
#############################################
pdf(paste0("stan_2g_mlr0_new_tr_",task_id,".pdf"))
