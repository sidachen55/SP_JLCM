################################################################################
# scenario: 4 sub-populations (G=4)
# split class 2 in the original G=3 setting into 2 subclasses
################################################################################
...
theta2 <- c(-9, 0.5, 10)
theta3 <- c(-6, 0.3, 10)
theta4 <- c(0.03, -0.01, -1.96)

sigma1.e <- sigma2.e <- sigma3.e <- sigma4.e <- 0.69
Sigma1.b <- diag(c(0.87, 0.02))
Sigma2.b <- diag(c(0.05, 0.95))
Sigma3.b <- diag(c(0.2, 0.65))
Sigma4.b <- diag(c(0.28, 0.31))
...
# Simulating from JM4
data4 <- sim_JM(n=n4, theta=theta4, sigma.e=sigma4.e, Sigma.b=Sigma4.b,
                beta=beta4, alpha=alpha4, lambda=lambda4, tmax=tmax, init=n1+n2+n3+1)

#### 
data <- list(longitudinal=rbind(data1$longitudinal, data2$longitudinal, data3$longitudinal, data4$longitudinal), survival=rbind(data1$survival, data2$survival, data3$survival, data4$survival))
