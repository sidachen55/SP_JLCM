################################################################################
# implement the overfitted JLCM in Andrinopoulou et al 2020
################################################################################
library(JMbayes)
library(rjags)
library(xtable)
...
surv_data_subset <- data$survival[, c("id", "time", "status", "x2")]
long_data <- merge(data$longitudinal, surv_data_subset, by = "id")


data <- data.frame(echotime=long_data$times, group=long_data$x1, age=long_data$x2, IDnr=long_data$id, y0=long_data$y, years=long_data$time, status=long_data$status)
data.id <- data.frame(IDnr=surv_data$id, years=surv_data$time, status=surv_data$status, age=surv_data$x2)
...
save(fit_smy, file=paste0("jags_om_",task_id,".rdata"))
