################################################################################
# for sim model1, S2 (G=2 clusters)
################################################################################
library(rstan)
library(loo)
library(statmod)
...
# Fit the joint latent class model with 2 latent classes
...
init_function <- function() {
  list(
    ...
    alpha1 = rnorm(1,0,1), 
    alpha2 = rnorm(1,0,1),
    theta1 = (matrix(fit_summary[,"coef"], 2, 3)[1,])* runif(1,0.7,1.4),
    theta2 = (matrix(fit_summary[,"coef"], 2, 3)[2,])* runif(1,0.7,1.4),
    psi1 = rnorm(1,0,1),
...
##########################################
fit <- stan(file = "rstan_2g_new.stan", 
...
loglik_allchain <- extract(fit, pars = "log_lik_cond", permuted = FALSE) # log_lik_cond changed to log_lik
...
save(fit_smy, file=paste0("stan_ms_2g_",task_id,".rdata"))


#############################################
# Create trace plots for specified parameters
#############################################
pdf(paste0("stan_ms_2g_tr_",task_id,".pdf"))
traceplot(fit, pars = c("var_e1", "var_e2", "psi1", "theta1", "theta2", "var_b1", "var_b2", "lp__", "log_posterior_original"), inc_warmup = F) 
dev.off()