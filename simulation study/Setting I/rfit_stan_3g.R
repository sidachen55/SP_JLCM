################################################################################
# for sim model1, S3 (G=3 clusters)
################################################################################
library(rstan)
library(loo)
library(statmod)
...
init_function <- function() {
  list(
    ...
    alpha2 = rnorm(1,0,1), 
    alpha3 = rnorm(1,0,1),
    theta1 = (matrix(fit_summary[,"coef"], 3, 3)[1,])* runif(1,0.7,1.4),
    theta2 = (matrix(fit_summary[,"coef"], 3, 3)[2,])* runif(1,0.7,1.4),
    theta3 = (matrix(fit_summary[,"coef"], 3, 3)[3,])* runif(1,0.7,1.4),
    psi1 = rnorm(1,0,1),
    psi2 = rnorm(1,0,1),
...
##########################################
fit <- stan(file = "rstan_3g_new.stan", 
...
save(fit_smy, file=paste0("stan_ms_3g_",task_id,".rdata"))


#############################################
# Create trace plots for specified parameters
#############################################
pdf(paste0("stan_ms_3g_tr_",task_id,".pdf"))
traceplot(fit, pars = c("var_e1", "var_e2", "var_e3", "psi1", "psi2", "theta1", "theta2", "theta3", "var_b1", "var_b2", "var_b3" ,"lp__" ,"log_posterior_original"), inc_warmup = F) 
dev.off()