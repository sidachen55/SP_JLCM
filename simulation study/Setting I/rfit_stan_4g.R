################################################################################
# for sim model1, S4 (G=4 clusters)
################################################################################
library(rstan)
library(loo)
library(statmod)
...
# Fit the joint latent class model with 4 latent classes
...
init_function <- function() {
  list(
    ...
    alpha3 = rnorm(1,0,1),
    alpha4 = rnorm(1,0,1),
    theta1 = (matrix(fit_summary[,"coef"], 4, 3)[1,])* runif(1,0.7,1.4),
    theta2 = (matrix(fit_summary[,"coef"], 4, 3)[2,])* runif(1,0.7,1.4),
    theta3 = (matrix(fit_summary[,"coef"], 4, 3)[3,])* runif(1,0.7,1.4),
    theta4 = (matrix(fit_summary[,"coef"], 4, 3)[4,])* runif(1,0.7,1.4),
    psi1 = rnorm(1,0,1),
    psi2 = rnorm(1,0,1),
    psi3 = rnorm(1,0,1),
...
##########################################
fit <- stan(file = "rstan_4g_new.stan", 
...
save(fit_smy, file=paste0("stan_ms_4g_",task_id,".rdata"))
